...
namespace Networking
{
	...
	int send_string_to_client(const char*string,uint8_t id,bool send_all){
		if(sclient==NULL){
...
// allocates more memory in case pkt->string is not big enough for string;
	...
	int send_vec2_to_client(float x,float y,uint8_t id,bool send_all) {
		if (sclient==NULL)
			return -1;
		packet_t pkt;
	...
	int send_int_to_client(int value,uint8_t id,bool send_all){
		if(sclient==NULL)
			return -1;
		packet_t pkt;
